...
public class mainGame {
	...
	public static void main(String[] args) {
		...
		while (inSession) {
			...
			if (check.isGameFinished(gameBoard)) {
				inSession = false;
				break;
			}
	...
	private static void playerTurn(examiner check, char[][] gameBoard, String p1) {
		...
		while(!check.inputValid(gameBoard, playerPos)) {
	...
	private static void cpu2Turn(examiner check, char[][] gameBoard) {
		...
		// checks to see if cpu is playing right move, so it doesn't overwrite player 1's moves
		while(true) {
